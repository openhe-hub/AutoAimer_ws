...
class DoFrameDifference {
 ...
 public:
  ...
  cv::Mat get_diff(const cv::Mat& s1,
                   const cv::Mat& s2,
                   const cv::Mat& ref,
...
class DetectBullet {
  ...
  double tme_get_brightest, tme_sort_points, tme_find_contours,
      tme_min_area_rect, tme_get_possible, tme_total;  // 记录使用时间
 ...
 public:
  ...
  std::vector<ImageBullet> process_new_frame(const cv::Mat& new_frame,
