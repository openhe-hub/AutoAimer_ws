...
auto AbManager::get_ab() const -> top4::AbArmor {
  return top4::AbArmor(this->length_filter.predict(0.)(0, 0),
                       this->z_filter.predict(0.)(0, 0));
}
...
void AbManager::update(const Eigen::Vector3d& center, const top4::AbArmor& d) {
  ...
  this->length_filter.update(
      d.length, 0., {1.},
      {aimer::param::find_double_param("TOP_AB_LENGTH_R_AT_1M") * dis * dis});
  this->z_filter.update(
      d.z_plus, 0., {1.},
      {aimer::param::find_double_param("TOP_AB_Z_R_AT_1M") * dis * dis});
}

double AbManager::get_cost(const top4::AbArmor& d) const {
  top4::AbArmor res = this->get_ab();
  return std::fabs(res.length - d.length) * 3. +
         std::fabs(res.z_plus - d.z_plus) * 10.;
}
...
RotateCounter::RotateCounter(const double& init_radius) {
  this->ab_manager[0].update(Eigen::Vector3d(1., 0., 0.),
                             top4::AbArmor(init_radius, 0.));
  this->ab_manager[1].update(Eigen::Vector3d(1., 0., 0.),
                             top4::AbArmor(init_radius, 0.));
}
...
bool RotateCounter::top_active() const {
  return !this->w_inactive() &&
         std::fabs(this->get_active_rotate()) >=
             aimer::param::find_int_param("TOP4_ACTIVE_ROTATE");
}

void RotateCounter::update_theta_l(const double& theta_l,
                                   const double& t,
                                   const double& R) {
...
void RotateCounter::update_ab(const Eigen::Vector3d& center,
  ...
  double cost = this->pattern == top4::TopPattern::AB ? cost_ab - cost_ba
                                                      : cost_ba - cost_ab;
  this->pattern_fixer.update(cost, t);
  if (this->pattern_fixer.get_x_k1()(0, 0) > 0.) {
    this->pattern = this->pattern == top4::TopPattern::AB
                        ? top4::TopPattern::BA
                        : top4::TopPattern::AB;
...
std::vector<TopArmor> TopModel::predict_armors(const double& t) const {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    Eigen::Vector3d pos{center(0, 0) + length * norm2(0, 0),
                        ...
                        center(2, 0) + z_plus};
    res.push_back(TopArmor(pos, t, aimer::math::reduced_angle(theta - theta_zn),
                           length, z_plus));
...
aimer::AimInfo TopModel::get_limit_aim() const {
  ...
  std::vector<TopArmor> prediction_results = this->predict_armors(
      this->converter->filter_to_prediction_time(this->center_filter));
  std::vector<TopArmor> hit_results = this->predict_armors(
...
void TopModel::update() {  // 1 or 2
  ...
  aimer::debug::flask_aim << fmt::format("top_w {:.2f}",
                                         this->counter.get_w() / M_PI * 180.);
  if (this->counter.get_pattern() == top4::TopPattern::A) {
    aimer::debug::flask_aim
        << fmt::format("len in {:.3f}|#{:.3f}", this->counter.get_ab(0).length,
    ...
    aimer::debug::flask_aim
        << fmt::format("len in {:.3f}|#{:.3f}", this->counter.get_ab(1).length,
...
auto TopModel::length_sampling(const std::vector<aimer::ArmorData>& armors,
  ...
  // 双板距离都可信，进行最大化更新
  double z_to_l_fit =
      top::fit_double_z_to_l(armors, z_to_l_exp, M_PI / 2., M_PI, converter);

  // // 在核心采样点复现圆心并对长轴和短轴进行采样
  Eigen::Vector2d l_pos2 = Eigen::Vector2d(armors[0].info.pos(0, 0),
                                           armors[0].info.pos(1, 0)),
                  r_pos2 = Eigen::Vector2d(armors[1].info.pos(0, 0),
                                           armors[1].info.pos(1, 0));
  Eigen::Vector2d center2 = aimer::math::get_intersection(
      l_pos2, aimer::math::rotate(converter->get_camera_z_i2(), z_to_l_fit),
      r_pos2,
      aimer::math::rotate(converter->get_camera_z_i2(),
                          z_to_l_fit + M_PI / 2.));
  Eigen::Vector3d center{
      center2(0, 0), center2(1, 0),
  ...
  // 绘制拟合 8 点
  {
    ...
    aimer::debug::flask_map << aimer::debug::poses_to_map_lines(
        {armors[0].info.pos, center, armors[1].info.pos}, {0, 255, 0}, false,
...
double TopModel::double_angle_sampling(
  ...
  double z_to_l_fit_by_r = [&]() {
    double z_to_r_fit = top::fit_single_z_to_v(armors[1], z_to_l_exp + M_PI / 2,
                                               M_PI, M_PI / 2. * 3., converter);
    return aimer::math::reduced_angle(z_to_r_fit - M_PI / 2.);
  }();
  // 若直接加权，需保证符号为正
  double z_to_l_fit_weight =
      aimer::math::get_weighted_angle(z_to_l_fit_by_l, armors[0].info.area(),
                                      z_to_l_fit_by_r, armors[1].info.area());
...
void TopModel::update_by_double_armors() {
  ...
  if (aimer::math::get_ratio(armors[0].info.area(), armors[1].info.area()) >
      aimer::param::find_double_param("TOP_RADIUS_SAMPLING_AREA_RATIO")) {
    ...
    aimer::debug::flask_aim << fmt::format(
        "zn_to_l[D]: {:.2f}",
        aimer::math::reduced_angle(length_sample.z_to_l_fit + M_PI) / M_PI *
            180.);
    aimer::debug::flask_aim
        << fmt::format("len sampLR {:.3f}|{:.3f}", length_sample.ab[0].length,
    ...
    this->counter.update_ab(length_sample.center, length_sample.ab,
                            this->converter->get_img_t());
  } else {
    ...
    this->counter.update_theta_l(
        ...
        this->converter->get_img_t(),
        aimer::param::find_double_param("TOP_DOUBLE_ANGLE_R"));
  }
  // 无论如何都要估计角度，更新中心
  ...
  Eigen::Vector3d center_pro =
      (z_to_l_fix > M_PI / 4. * 3.)
          ? ([&]() {  // 懒得传参数
              Eigen::Vector2d l_norm2 = aimer::math::rotate(
                  this->converter->get_camera_z_i2(), z_to_l_fix);
              // fix !
              int l_index =
                  this->counter.get_pattern() == top4::TopPattern::AB ? 0 : 1;
              return top::prolonged_center(
                  armors[0].info.pos, this->counter.get_ab(l_index).length,
                  l_norm2, this->counter.get_ab(l_index).z_plus);
            }())
          : ([&]() {
              Eigen::Vector2d r_norm2 = aimer::math::rotate(
                  this->converter->get_camera_z_i2(), z_to_l_fix + M_PI / 2.);
              int r_index =
                  this->counter.get_pattern() == top4::TopPattern::AB ? 1 : 0;
              return top::prolonged_center(
                  armors[1].info.pos, this->counter.get_ab(r_index).length,
                  r_norm2, this->counter.get_ab(r_index).z_plus);
            }());
  this->center_filter.update(center_pro, this->converter->get_img_t(),
                             {0.01, 10.},
...

  {
    ...
    aimer::debug::flask_aim
        << aimer::debug::FlaskPoint(cpt, {0, 255, 255}, 6, 3);
  }
}
...
void TopModel::update_by_single_armor() {
  ...
  double z_to_l_exp = aimer::math::reduced_angle(
      this->counter.predict_theta_l(this->converter->get_img_t()) -
      this->converter->get_camera_z_i_yaw());
  double z_to_l_fit = top::fit_single_z_to_v(armor, z_to_l_exp, M_PI / 4 * 3,
                                             M_PI / 4 * 5, this->converter);
  ...
  Eigen::Vector3d center_pro = [&]() {
    ...
    return top::prolonged_center(armor.info.pos,
                                 this->counter.get_ab(l_index).length, l_norm2,
...
void TopModel::draw_aim(cv::Mat& img) const {
  // 获取此刻结果
  std::vector<TopArmor> res = this->predict_armors(
      this->converter->get_img_t());  // 包含 Pattern 的具体实现
                                      // std::vector<TopArmor> res =
