...
FilterThread::FilterThread(aimer::CoordConverter* const converter,
    ...
    : filter(), armor(armor), credit_clock(converter, credit_time) {
  // 确认更新即可 update clock
  this->credit_clock.update();
  this->filter.set_pos(aimer::math::xyz_to_ypd(armor.info.pos));
}
...
void FilterThread::update(const aimer::ArmorData& armor, const double& t) {
  ...
  std::vector<double> q_vec = []() {
    double q_x = aimer::param::find_double_param("MOTION_EKF_Q_X");
    double q_v = aimer::param::find_double_param("MOTION_EKF_Q_V");
    return std::vector<double>{q_x, q_v, q_x, q_v, q_x, q_v};
  }();
  std::vector<double> r_vec{
      aimer::param::find_double_param("MOTION_EKF_R_YAW"),
      aimer::param::find_double_param("MOTION_EKF_R_PITCH"),
      aimer::param::find_double_param("MOTION_EKF_R_DISTANCE_AT_1M") * ypd.dis *
          ypd.dis};
...
/** @class MotionModel */
MotionModel::MotionModel(aimer::CoordConverter* const converter,
                         aimer::EnemyState* const state,
                         const double& credit_time)
    : converter(converter), state(state), credit_time(credit_time) {}

void MotionModel::draw_aim(cv::Mat& img) const {
  for (auto& d : this->filters) {
    double prediction_t =
        this->converter->filter_to_prediction_time(d.second.filter);
    Eigen::Vector3d pos =
        d.second.filter.predict_pos(this->converter->get_img_t());
    Eigen::Vector3d v = d.second.filter.predict_v(prediction_t);
    cv::Scalar color = this->state->is_hit() ? aimer::debug::GRAY_MAIN_COLOR
                       : this->converter->get_robot_status_ref().enemy_color ==
                               ::EnemyColor::BLUE
                           ? aimer::debug::BLUE_MAIN_COLOR
                           : aimer::debug::RED_MAIN_COLOR;
    aimer::debug::flask_map << aimer::debug::pos_to_map_point(
        pos, color, aimer::debug::FLASK_MAP_PT_RADIUS,
        aimer::debug::FLASK_MAP_THICKNESS);
    aimer::debug::flask_map << aimer::debug::pos_pair_to_map_arrow(
        std::make_pair(pos, pos + v * 1.), color,
        aimer::debug::FLASK_MAP_THICKNESS);
    aimer::debug::flask_map << aimer::debug::pos_str_to_map_text(
        fmt::format("{}", this->state->get_number()), pos, color,
  ...
  for (auto& d : armors) {
    ...
    if (it == this->filters.end()) {
      this->filters.insert(std::make_pair(
          d.id, FilterThread(this->converter, d, this->credit_time)));
...
// 根据线程死活进行判断是否 idle
aimer::AimInfo MotionModel::get_aim(const GetAimOption& option) const {
  if (option.passive)
    return this->get_passive_aim();
  return this->get_positive_aim();
}

aimer::AimInfo MotionModel::get_positive_aim() const {
  double max_area = 0.;
  for (auto& d : this->filters)
    if (d.second.credit() && d.second.armor.info.area() > max_area)
      max_area = d.second.armor.info.area();
  aimer::AimInfo aim =
      aimer::AimInfo::idle();  // IDLE 弱于 TRACKING 和 SHOOT_NOW
  for (auto& d : this->filters) {
    if (!d.second.credit() ||
        d.second.armor.info.area() < max_area * aimer::param::find_double_param(
                                                    "MOTION_AIM_AREA_RATIO")) {
      continue;
    ...
    aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
      ...
      aimer::math::YpdCoord ypd_v =
          this->converter->filter_to_aim_ypd_v(d.second.filter);
      return aimer::AimInfo(
          ypd, ypd_v, shoot_param,
          this->converter->aim_error_exceeded(
              ypd, this->state->get_sample_armor_ref(),
              this->state->get_aim_error(), 0., this->state->get_armor_pitch())
              ? ::ShootMode::TRACKING
              : ::ShootMode::SHOOT_NOW);
    }();
...
aimer::AimInfo MotionModel::get_passive_aim() const {
  double max_area = 0.;
  for (auto& d : this->filters)
    if (d.second.credit() && d.second.armor.info.area() > max_area)
      max_area = d.second.armor.info.area();
  ...
  for (auto& d : this->filters) {
    if (!d.second.credit() ||
        d.second.armor.info.area() <
            max_area * aimer::param::find_double_param("MOTION_AIM_AREA_RATIO"))
      continue;  // 面积小于最大 0.5 以下的不打，然后按照中心排序
    aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
      Eigen::Vector3d hit_pos =
          this->converter->filter_to_hit_pos(d.second.filter);
      aimer::ShootParam shoot_param =
          this->converter->target_pos_to_shoot_param(hit_pos);
      aimer::math::YpdCoord ypd =
          this->converter->aim_xyz_i_to_aim_ypd(shoot_param.aim_xyz_i_barrel);
      aimer::math::YpdCoord ypd_v =
          this->converter->filter_to_hit_aim_ypd_v(d.second.filter);
      return aimer::AimInfo(
          {this->converter->get_control_aim_yaw0(), ypd.pitch, ypd.dis},
          {0., 0., ypd_v.dis}, shoot_param,
          // 若仅考虑 yaw 就超误差，不跟随
          this->converter->aim_error_exceeded(
              aimer::math::YpdCoord(
                  ypd.yaw, this->converter->get_control_aim_pitch0(), ypd.dis),
              this->state->get_sample_armor_ref(),
              aimer::param::find_double_param(
                  "MOTION_PASSIVE_MODE_AIM_TRACKING_RANGE"),
              0., this->state->get_armor_pitch())
              ? ::ShootMode::IDLE
          // 若 yaw 不超误差
          ...
          : this->converter->aim_error_exceeded(
                ypd, this->state->get_sample_armor_ref(),
                aimer::param::find_double_param(
                    "MOTION_PASSIVE_MODE_AIM_MAX_ERROR"),
                0., this->state->get_armor_pitch())
              ? ::ShootMode::TRACKING
              : ::ShootMode::SHOOT_NOW);
    }();
