   ...
   private:
    ...
    void draw_boxes(cv::Mat& src, const std::vector<bbox_t>& results) {
        ...
        for (const auto& box : results) {
            ...
            cv::putText(src, std::to_string(box.tag_id), box.pts[0], 0, 1,
                        color);
    ...
    // 根据离装甲板中心距离确定
    static bool cmp_armor_center(aimer::DetectedArmor a1, aimer::DetectedArmor a2) {
    ...
    cv::Point2d getCenter(aimer::DetectedArmor a) {
   ...
   public:
    ...
    void detector_run(const std::string& module_path, ros::NodeHandle& node) {
        ...
        this->sensorSub = node.subscribe<aimer_msgs::SensorsData>(
            "sensors_data", 10,
            boost::bind(&DetectorOpencv::sensorSubCallback, this, _1, module));
        this->detectionPub =
    ...
    void publishDetectionResult(aimer::DetectionResult result) {
        ...
        Eigen::Quaterniond _q = {(double)vec(0), (double)vec(1), (double)vec(2),
                                 (double)vec(3)};
