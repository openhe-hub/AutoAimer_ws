...
void publishDetectionResult(const ros::Publisher &publisher, aimer::DetectionResult result);
...
void detector_run(const std::string& module_path) {
    ...
    while (true) {
        try {
            ...
            // 将检测结果放入FIFO
            publishDetectionResult({image,q,timestamp,armors});
...
void sensorSubCallback(const aimer_msgs::SensorsData::ConstPtr& data) {
    ...
    cv_bridge::CvImagePtr cv_ptr =
        cv_bridge::toCvCopy(data->img, sensor_msgs::image_encodings::TYPE_8UC3);
    cv::Mat img = cv_ptr->image;
    // time => double
    double timestamp = data->stamp.toSec();
    // geometry_msgs/Quaternion => Eigen Quaternion
    Eigen::Quaterniond q;
    tf::quaternionMsgToEigen(data->q, q);
...
void publishDetectionResult(const ros::Publisher &publisher, aimer::DetectionResult result){
    ...
    sensor_msgs::ImagePtr imagePtr=cv_bridge::CvImage(std_msgs::Header(),"bgr8",result.img).toImageMsg();
    _result.img= *imagePtr;
    // double => time
    _result.stamp=ros::Time().fromSec(result.timestamp);
    // Eigen Quaterniond => geomotry_msgs/Quaternion
    tf::quaternionEigenToMsg(result.q, _result.q);
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber sensorSub = node.subscribe<aimer_msgs::SensorsData>(
        "sensors_data", 10, sensorSubCallback);
    ros::Publisher detectionPub =
        node.advertise<aimer_msgs::DetectionResult>("detection_result", 10);

    ros::Publisher imgPub=node.advertise<sensor_msgs::Image>("image-detection",10);
