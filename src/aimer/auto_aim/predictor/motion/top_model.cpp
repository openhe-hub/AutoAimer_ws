...
double get_pts_cost(const std::vector<cv::Point2f>& cv_refs,
  ...
  for (std::size_t i = 0u; i < size; ++i) {
    refs.push_back(Eigen::Vector2d(cv_refs[i].x, cv_refs[i].y));
    pts.push_back(Eigen::Vector2d(cv_pts[i].x, cv_pts[i].y));
  }
  double cost = 0.;
  for (std::size_t i = 0u; i < size; ++i) {
    ...
    double pixel_dis =  // dis 是指方差平面内到原点的距离
        ...
        ref_d.norm();
    double angular_dis =
        ref_d.norm() * aimer::math::get_abs_angle(ref_d, pt_d) / ref_d.norm();
    // 平方可能是为了配合 sin 和 cos
    // 弧度差代价（0 度左右占比应该大）
    double cost_i = math::sq(pixel_dis * std::sin(inclined)) +
                    math::sq(angular_dis * std::cos(inclined)) *
                        aimer::DETECTOR_ERROR_PIXEL_BY_SLOPE;
    // 重投影像素误差越大，越相信斜率
...
std::vector<Eigen::Vector3d> radial_armor_corners(
  ...
  Eigen::Vector2d radius_norm =
      aimer::math::rotate(converter->get_camera_z_i2(), z_to_v);
...
std::vector<std::vector<cv::Point2f>> radial_double_pts(
  ...
  res.push_back(top::radial_armor_pts(armors[0].info.pos,
                                      armors[0].info.sample.type,
                                      armors[0].info.pitch, z_to_l, converter));
  res.push_back(top::radial_armor_pts(
      armors[1].info.pos, armors[1].info.sample.type, armors[0].info.pitch,
      z_to_l + M_PI / 2., converter));
  return res;
}
...
double SingleCost::operator()(const double& x) {
  ...
  std::vector<cv::Point2f> pts = top::radial_armor_pts(
      ...
      this->data.d.info.pitch, x, this->data.converter);
  return top::get_pts_cost(pts,
                           std::vector<cv::Point2f>(this->data.d.info.pus,
                                                    this->data.d.info.pus + 4),
...
double DoubleCost::operator()(const double& x) {
  ...
  return top::get_pts_cost(
             ml_pts[0],
             std::vector<cv::Point2f>(this->data.armors[0].info.pus,
                                      this->data.armors[0].info.pus + 4),
             this->data.z_to_l_exp) +
         top::get_pts_cost(
             ml_pts[1],
             std::vector<cv::Point2f>(this->data.armors[1].info.pus,
                                      this->data.armors[1].info.pus + 4),
             z_to_r_exp);
}
...
double fit_single_z_to_v(const aimer::ArmorData& armor,
                         ...
                         aimer::CoordConverter* const converter) {
  top::SingleCost single_cost =
      top::SingleCost(top::SingleData(armor, z_to_v_exp, converter));
  aimer::math::Trisection solver;
  std::pair<double, double> res = solver.find(
      z_to_v_min, z_to_v_max, single_cost, aimer::FIND_ANGLE_ITERATIONS);
...
double fit_double_z_to_l(const std::vector<aimer::ArmorData>& armors,
                         ...
                         aimer::CoordConverter* const converter) {
  top::DoubleCost double_cost{
      top::DoubleCost(top::DoubleData(armors, z_to_l_exp, converter))};
  aimer::math::Trisection solver;
  std::pair<double, double> res = solver.find(
      z_to_l_min, z_to_l_max, double_cost, aimer::FIND_ANGLE_ITERATIONS);
  return aimer::math::reduced_angle(res.first);
}
...
aimer::AimInfo get_top_limit_aim(
  ...
  struct AimArmor {
    AimArmor(const aimer::AimInfo& emerging_aim, const top::TopArmor& armor)
        : emerging_aim(emerging_aim), armor(armor) {}
  ...
  auto get_directs = [](const std::vector<top::TopArmor>& results)
      -> std::vector<top::TopArmor> {
    std::vector<top::TopArmor> directs;
    for (auto& res : results) {
      if (std::fabs(res.zn_to_v) <
          aimer::math::deg_to_rad(aimer::param::find_double_param(
              "TOP_AIM_MAX_ORIENTATION_ANGLE"))) {
        directs.push_back(res);
      }
    }
    return directs;
  };
  auto choose_direct_aim =
      [&converter, &state, &center_filter, &top_w,
       &get_direction](const std::vector<top::TopArmor>& directs) -> AimArmor {
    ...
    for (auto& d : directs) {
      ...
      if (converter->aim_swing_cost(converter->target_pos_to_aim_ypd(d.pos)) <
          converter->aim_swing_cost(
              converter->target_pos_to_aim_ypd(direct.pos))) {
        direct = d;
      }
    }
    aimer::AimInfo aim = [&]() {
  ...
  // results 应该包含时间
  auto get_indirect_aim =
      [&converter, &state, &center_filter, &top_w,
       &get_direction](const std::vector<top::TopArmor>& results) -> AimArmor {
    double zn_to_lim = get_direction(top_w) == top::TopDirection::CCW
                           ? -aimer::param::find_double_param(
                                 "TOP_AIM_MAX_ORIENTATION_ANGLE") /
                                 180. * M_PI
                           : +aimer::param::find_double_param(
                                 "TOP_AIM_MAX_ORIENTATION_ANGLE") /
    ...
    for (auto& armor : results) {
      ...
      double leaving_angle =
          (state->get_sample_armor_ref().width / 2. *
           aimer::param::find_double_param("TOP_AIM_MAX_OUT_ERROR")) /
          armor.length;
      double armor_to_lim =
          aimer::math::reduced_angle((get_direction(top_w) == TopDirection::CCW
                                          ? zn_to_lim - armor.zn_to_v
    ...
    // lim 度（例如 0）时的中心（欲瞄准
    // 用这个中心延长的位置）
    Eigen::Vector3d center_lim = center_filter.predict_pos(
        results[0].t + closest_to_lim / std::fabs(top_w));
    // what if top_w == 0?
    ...
    Eigen::Vector3d emerging_pos = {emerging_pos2(0, 0), emerging_pos2(1, 0),
                                    center_lim(2, 0) + indirect.z_plus};
    aimer::AimInfo aim = [&]() {
      aimer::ShootParam shoot_param =
          converter->target_pos_to_shoot_param(emerging_pos);
      aimer::math::YpdCoord ypd =
          converter->aim_xyz_i_to_aim_ypd(shoot_param.aim_xyz_i_barrel);
      aimer::math::YpdCoord ypd_v =
          converter->filter_to_aim_ypd_v(center_filter);
  ...
  auto get_aim_result =
      [&get_directs, &choose_direct_aim, &get_indirect_aim](
          const std::vector<top::TopArmor>& results) -> AimArmor {
    std::vector<top::TopArmor> directs = get_directs(results);
    return !directs.empty() ? choose_direct_aim(directs)
                            : get_indirect_aim(results);
  };

  aimer::AimInfo tracking_aim = get_aim_result(prediction_results).emerging_aim;
  {  // 以下判断 shoot
    // 常规 motion_model 的判断发弹为：当前是否足够收敛（理想 yaw 始终为 0）
    ...
    // 2. hit: 如果即使收敛，收敛位置不适合发射子弹，则不发射（主要对于非 LIM
    // 范围装甲板的打击限制）
    // 无法根据对 hit 收敛程度判断是否发射，hit 没有收敛之说 可否通过对
    // 3. prediction 收敛程度判断是否发射？针对初始跟随和中间跟随。
    // 注意 prediction 和 hit 在切换临界处根本就是两个位置
    AimArmor hit_aim = get_aim_result(hit_results);
    bool for_a_hit =
        converter->aim_error_exceeded(
            tracking_aim.ypd, state->get_sample_armor_ref(),
            state->get_aim_error(), hit_aim.armor.zn_to_v,
            state->get_armor_pitch()) ||
                converter->aim_error_exceeded(
                    hit_aim.emerging_aim.ypd, state->get_sample_armor_ref(),
                    aimer::param::find_double_param("TOP_AIM_MAX_SWING_ERROR"),
                    hit_aim.armor.zn_to_v, state->get_armor_pitch()) ||
                // 对于英雄，考虑敌人原地陀螺，
                // 就算 appearing 已经跟紧了，tracking
                // 当然也跟紧了，收敛偏差都是 0 了 仍然不能发子弹
                // 但是不能用枪口的 yaw 去判断，考虑装甲板切换回转的时候...
                converter->aim_error_exceeded(
            ...
            : true;
    tracking_aim.shoot =
        for_a_hit ? ::ShootMode::SHOOT_NOW : ::ShootMode::TRACKING;
  }
  return tracking_aim;
}
...
void top_draw_aim(cv::Mat& img,
  ...
  aimer::debug::flask_map << aimer::debug::pos_str_to_map_text(
      fmt::format("{}", state->get_number()), center,
      aimer::debug::FLASK_MAP_TEXT_COLOR, aimer::debug::FLASK_MAP_TEXT_SCALE);

  std::vector<std::vector<cv::Point2f>> pts_vec;
  std::vector<std::vector<Eigen::Vector3d>> armors_corners;
  for (auto& d : top_results) {
    pts_vec.push_back(top::radial_armor_pts(
        d.pos, state->get_sample_armor_ref().type, state->get_armor_pitch(),
        d.zn_to_v + M_PI, converter));
  ...
  for (int i = 0; i < size; ++i) {
    ...
    aimer::debug::draw_lines(img, pts_vec[i], color_armor * coe_i, thickness,
                             true);
    aimer::debug::draw_line(img, pts_vec[i][3], pts_vec[p][0],
                            color_sp * ((coe_i + coe_p) / 2.), thickness);
    aimer::debug::draw_line(img, pts_vec[i][2], pts_vec[p][1],
                            color_sp * ((coe_i + coe_p) / 2.), thickness);
    aimer::debug::flask_map << aimer::debug::poses_to_map_lines(
        armors_corners[i],
        state->is_hit() ? aimer::debug::GRAY_MAIN_COLOR : color_armor, true,
        aimer::debug::FLASK_MAP_THICKNESS);
    aimer::debug::flask_map << aimer::debug::poses_to_map_lines(
        {armors_corners[i][3], armors_corners[p][0]},
        state->is_hit() ? aimer::debug::GRAY_MAIN_COLOR : color_sp, false,
        aimer::debug::FLASK_MAP_THICKNESS);
    aimer::debug::flask_map << aimer::debug::poses_to_map_lines(
 ...
 *
 */
SimpleRotateCounter::SimpleRotateCounter(const int& armor_cnt,
...
double SimpleRotateCounter::predict_theta_v(const double& t) const {
  ...
  return aimer::math::reduced_angle(
      theta_s - this->rotate * (M_PI * 2. / this->armor_cnt));
}

bool SimpleRotateCounter::w_active() const {  // 旋转状态
  return std::fabs(this->get_w()) >
         aimer::param::find_double_param("TOP_ACTIVE_W") / 180. * M_PI;
}

bool SimpleRotateCounter::w_inactive() const {
  return std::fabs(this->get_w()) <=
         aimer::param::find_double_param("TOP_INACTIVE_W") / 180. * M_PI;
}

bool SimpleRotateCounter::top_active() const {
  return !this->w_inactive() &&
         std::fabs(this->get_active_rotate()) >= this->min_active_rotate;
}

void SimpleRotateCounter::update_theta_v(const double& theta_v,
                                         const double& t,
                                         const double& R) {
  this->theta_s_filter.update(
      theta_v + this->rotate * (M_PI * 2. / this->armor_cnt), t, R);
}

// 在 update_rotate 之前并不知道 predict_v 是否真的是 l
void SimpleRotateCounter::update_rotate(const aimer::ArmorData& d,
  ...
  int seg =
      (std::fabs(zn_to_v) > this->jump_angle ? 1 : 0) * (zn_to_v > 0. ? 1 : -1);
...
OrientationSignFixer::OrientationSignFixer(
...
double OrientationSignFixer::fixed(const double& angle,
                                   const double& yaw) const {
  if (std::fabs(angle) <= this->reserving_range)
    return angle;
  double credit_mid_yaw = aimer::math::get_weighted_angle(
      this->credit_min_yaw, 0.5, this->credit_max_yaw, 0.5);
...
void OrientationSignFixer::update(const double& yaw, const double& t) {
  ...
  if (this->update_t - this->init_t > this->sampling_time) {
    // 本帧暂时不知道是不是拓展
    // 自身可信，回头即重置
    double early_to_late = aimer::math::get_rotate_angle(
        this->reserver.get_early(), this->reserver.get_late());
    double mid = aimer::math::get_weighted_angle(
        this->reserver.get_early(), 0.5, this->reserver.get_late(), 0.5);
    double mid_to_cur = aimer::math::get_rotate_angle(mid, yaw);
    if (mid_to_cur * early_to_late < 0.) {
      // 新周期，重置存储器
      // wtf!!!!
      this->credit_max_yaw = aimer::math::max_angle(this->reserver.get_early(),
                                                    this->reserver.get_late());
      this->credit_min_yaw = aimer::math::min_angle(this->reserver.get_early(),
                                                    this->reserver.get_late());
      this->reserver.init(yaw);
      this->init_t = t;
    }
...
SimpleTopModel::SimpleTopModel(aimer::CoordConverter* const converter,
                               aimer::EnemyState* const state,
                               const int& armor_cnt,
                               const double& jump_angle,
...
void SimpleTopModel::update(const UpdateOption& option) {
  const std::vector<aimer::ArmorData>& armors =
      this->state->get_armor_data_ref();
  // 更新失败
  ...
  aimer::ArmorData tracking_armor = armors[0];  // 设置默认值
  {
    ...
    for (auto& d : armors) {
      if (d.id == this->tracking_id)
        continue;
      if (d.id == max_id &&
          d.info.area() > id_area * aimer::SIMPLE_TOP_TRACK_AREA_RATIO) {
  ...
  double zn_to_v_raw = [&]() {
    double zn_to_v = aimer::math::reduced_angle(
        top::fit_single_z_to_v(tracking_armor, M_PI / 4., M_PI / 2.,
                               M_PI / 2. * 3., this->converter) -
        M_PI);
    // 有时正负颠倒，但不影响 fabs 判断
    if (option.enable_sign_fixer)
      return this->orientation_sign_fixer.fixed(
          zn_to_v, aimer::math::xyz_to_ypd(tracking_armor.info.pos).yaw);
    return zn_to_v;
  }();

  this->counter.update_rotate(tracking_armor, zn_to_v_raw,
                              this->converter->get_img_t());
  ...
  // 获取和更新角度
  {
    double z_to_v_exp = aimer::math::reduced_angle(
        ...
        this->converter->get_camera_z_i_yaw());  // reserve for debug
    double z_to_v_fit = aimer::math::reduced_angle([&]() {
      double zn_to_v = aimer::math::reduced_angle(
          top::fit_single_z_to_v(tracking_armor, z_to_v_exp, M_PI / 2.,
                                 M_PI / 2. * 3., this->converter) -
          M_PI);
      if (option.enable_sign_fixer)
        return this->orientation_sign_fixer.fixed(
    ...
    this->counter.update_theta_v(
        ...
        this->converter->get_img_t(),
        aimer::param::find_double_param("TOP_SINGLE_ANGLE_R"));
    // }
  }
  // 计算和更新中心
  {
    ...
    Eigen::Vector3d center_pro =
        top::prolonged_center(tracking_armor.info.pos,
                              this->state->get_default_radius(), l_norm2, 0.);
    this->center_filter.update(
        center_pro, this->converter->get_img_t(), {0.01, 10.},
        {aimer::param::find_double_param("TOP_CENTER_R")});
  }
}

std::vector<top::TopArmor> SimpleTopModel::predict_armors(
  ...
  for (int i = 0; i < this->armor_cnt; ++i) {
    ...
    Eigen::Vector3d pos{center(0, 0) + radius * norm2(0, 0),
                        center(1, 0) + radius * norm2(1, 0), center(2, 0)};
    res.push_back(top::TopArmor(
        pos, t, aimer::math::reduced_angle(theta - theta_zn), radius, 0.));
...
aimer::AimInfo SimpleTopModel::get_limit_aim() const {
  ...
  std::vector<top::TopArmor> prediction_results = this->predict_armors(
      this->converter->filter_to_prediction_time(this->center_filter));
  std::vector<top::TopArmor> hit_results = this->predict_armors(
      this->converter->filter_to_hit_time(this->center_filter));
  return top::get_top_limit_aim(this->center_filter, prediction_results,
                                hit_results, this->counter.get_w(),
                                this->converter, this->state);
}

void SimpleTopModel::draw_aim(cv::Mat& img) const {
  std::vector<top::TopArmor> res =
      this->predict_armors(this->converter->get_img_t());
  // 获取延迟结果
  // std::vector<top::TopArmor> res = this->predict_armors(
  //     converter->get_img_t() +
  //     converter->get_fire_to_hit_latency(aim_center.dis), converter);
  top::top_draw_aim(img, this->center_filter, res, this->active(),
                    this->converter, this->state);
}
