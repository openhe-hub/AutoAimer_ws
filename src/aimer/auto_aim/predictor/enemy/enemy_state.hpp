...
struct EnemyInfo {
  // 根据常数列表生成的常量表格
  EnemyInfo(const aimer::SampleArmor& sample_armor,
                   const double& armor_pitch,
                   const int& seen_armors_max_num,
                   const double& default_radius,
                   const char* aim_error_param)
      : sample_armor(sample_armor),
        armor_pitch(armor_pitch),
        seen_armors_max_num(seen_armors_max_num),
        default_radius(default_radius),
        aim_error_param(aim_error_param) {}
...
class EnemyState {
 public:
  ...
  void update(const std::vector<aimer::ArmorData>& raw_armors,
              const double& t,
              const int& frame);
 ...
 private:
  ...
  // 根据指定 size 裁剪和排序装甲板
  std::vector<aimer::ArmorData> sorted_armors(
