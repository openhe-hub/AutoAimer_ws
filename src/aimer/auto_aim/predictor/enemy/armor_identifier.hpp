...
class LightThread {
 public:
  ...
  // 是否存活
  bool alive(const ::EnemyColor& enemy_color, const double& t) const;

  // （如果认为是同一块装甲板）更新 Light 线程信息
  void update(const aimer::ArmorInfo& armor_info,
              const double& t,
              const int& frame);
...
class LightManager {
 public:
  LightManager(aimer::CoordConverter* const converter) : converter(converter) {}
