...
auto AimHistory::find_by_img_t(const double& img_t) const
    -> aim::IdTLatencyAimCorrection {
  // 比如发射延迟有 3s，但是只要发射时的枪口朝向能打到人就行
  auto it = std::lower_bound(this->aims.begin(), this->aims.end(), img_t,
                             // lower_bound: 第一个违背
                             [](const aim::IdTLatencyAimCorrection& a,
                                const double& t) { return a.img_t < t; });
  if (it == this->aims.end()) {
...
auto AimHistory::find_by_id(const int& id) const
    -> aim::IdTLatencyAimCorrection {
  auto it = std::lower_bound(this->aims.begin(), this->aims.end(), id,
                             [](const aim::IdTLatencyAimCorrection& a,
                                const int& id) { return a.id < id; });
...
auto ProjectileSimulator::get_aim_ref() const
    -> const aim::IdTLatencyAimCorrection& {
  return this->aim;
}
...
auto ProjectileSimulator::get_pos_by_t(const double& t) const -> aim::HitPos {
  ...
  double w =
      (t - this->fire_t) * shoot_param.v0 * std::cos(shoot_param.aim_angle);
  // 高度为微分方程结果
  double h = (k * shoot_param.v0 * std::sin(shoot_param.aim_angle) + this->g) *
                 k * w /
                 (k * k * shoot_param.v0 * std::cos(shoot_param.aim_angle)) +
             this->g *
                 std::log(1. - (k * w) / (shoot_param.v0 *
                                          std::cos(shoot_param.aim_angle))) /
                 k / k;
  // 弹道轨迹仅取决于目标点
  Eigen::Vector3d target_xyz_i_barrel =
      this->converter->xyz_i_camera_to_xyz_i_barrel(
          shoot_param.target_xyz_i_camera);
  Eigen::Vector3d w_norm =
      Eigen::Vector3d(target_xyz_i_barrel(0, 0), target_xyz_i_barrel(1, 0), 0)
          .normalized();
  Eigen::Vector3d h_norm = {0., 0., 1.};
  Eigen::Vector3d bullet_xyz_i_barrel = w * w_norm + h * h_norm;
  Eigen::Vector3d bullet_xyz_i_camera =
      this->converter->xyz_i_barrel_to_xyz_i_camera(bullet_xyz_i_barrel);
  Eigen::Vector2d bullet_xy_i_barrel = {bullet_xyz_i_barrel(0, 0),
                                        bullet_xyz_i_barrel(1, 0)};
  Eigen::Vector2d target_xy_i_barrel = {target_xyz_i_barrel(0, 0),
...
auto ProjectileSimulator::get_circle_by_t(const double& t) const
    -> aim::HitCircle {
  ...
  Eigen::Vector3d crossed =
      Eigen::Vector3d(0., 1., 0.).cross(xyz_c).normalized();
  Eigen::Vector3d edge_xyz_c =
      xyz_c + crossed * aimer::param::find_double_param("BULLET_RADIUS");
...
auto ProjectileSimulator::fit_circle(const aimer::math::CircleF& circle) const
  ...
  double t = solver
                 .find(this->fire_t, this->fire_t + aim::FIT_CIRCLE_MAX_T,
                       cost_func, aim::FIT_CIRCLE_ITERATIONS_NUM)
                 .first;
...
auto AimCorrector::update_bullet_id(const int& last_shoot_id) -> void {
  if (last_shoot_id != this->last_shoot_id) {
    ...
    if (this->pending_ids.size() + 1u <=
        aim::AIM_CORRECTOR_PENDING_IDS_MAX_SZ) {
  ...
  while (true) {
    ...
    if (fire_controlling_t + aim::AIM_CORRECTOR_AIM_TIME_MAX_ERROR <
        fire_controlling_aim.img_t) {
      this->pending_ids.pop();
      continue;
    }
    if (this->bullets.size() + 1u <= aim::AIM_CORRECTOR_BULLETS_MAX_SZ) {
      ...
      this->bullets.push_back(
          aim::IdProj{origin_aim.id,
                      aim::ProjectileSimulator(
                          this->converter, fire_controlling_aim,
...
auto AimCorrector::get_circles() -> std::vector<aim::IdCircle> {
  ...
  for (auto it = this->bullets.begin(); it != this->bullets.end();) {
    ...
    aim::HitCircle hit_circle =
        it->proj.get_circle_by_t(this->converter->get_img_t());
    if (hit_circle.hit) {
      it = this->bullets.erase(it);
    } else {
...
auto AimCorrector::sample_aim_errors() -> void {
  ...
  for (const auto& bullet : this->bullets) {
    ...
    for (auto it = undistorted_detected.begin();
         it != undistorted_detected.end(); ++it) {
      aim::CaughtCost caught_cost = bullet.proj.catch_circle(*it);
      if (caught_cost.caught && (caught_cost.caught > best_caught.caught ||
    ...
    if (best_bullet != undistorted_detected.end()) {
      ...
      aimer::math::YpdCoord caught_yp =
          this->converter->pu_to_yp_c(best_bullet->center);
      // 实际 - 预期（理想发射）
      Eigen::Vector2d yp_error = {caught_yp.yaw - fit_yp.yaw,
                                  caught_yp.pitch - fit_yp.pitch};
      {
        std::vector<double> r_vec = {
            aimer::param::find_double_param("AIM_CORRECTOR_ERROR_R")};
        const Eigen::Vector2d& correction =
            bullet.proj.get_aim_ref().correction;
        // 理想的校正后，yp_error = 0，而 aim_ref 中是之前的 error
        this->error_filters[0].update(yp_error[0] + correction(0, 0), 0., {1.},
                                      r_vec);
        this->error_filters[1].update(yp_error[1] + correction(1, 0), 0., {1.},
                                      r_vec);
      }

      if (this->error_angles.size() + 1u >
              aim::AIM_CORRECTOR_ERROR_ANGLES_MAX_SZ &&
  ...
  }
  {
    ...
    for (auto& d : this->error_angles) {
      ...
      aimer::debug::auto_aim_page()
          ...
          .get() =
          fmt::format("{:.2f}, {:.2f}", aimer::math::rad_to_deg(d(0, 0)),
...
auto AimCorrector::undistorted_circle(const aimer::math::CircleF& circle)
    -> aimer::math::CircleF {
  std::vector<float> rs;
  cv::Point2f circle_u = this->converter->pd_to_pu(circle.center);
  const cv::Point2f di[4] = {{1.f, 0.f}, {0.f, 1.f}, {-1.f, 0.f}, {0.f, -1.f}};
  for (int k = 0; k < 4; ++k) {
    cv::Point2f edge = circle.center + di[k] * circle.r;
