...
class PeriodicRecorder {
 public:
  PeriodicRecorder(const std::function<bool(const ValueT&, const ValueT&)>& cmp,
      ...
      : cmp(cmp), stamp(init_stamp) {}
  void update(const ValueT& value,
              const PeriodT& stamp,
              const PeriodT& period) {
  ...
  ValueT get() const { return this->last_record; }
...
class PeriodicAverage {
 public:
  PeriodicAverage(const std::size_t& buf_size, const PeriodT& init_stamp)
      : buf_size(buf_size), stamp(init_stamp) {}
  ...
  ValueT get() const { return this->last_period_ave; }
...
struct FlaskPoint {
  FlaskPoint(const cv::Point2f& pt,
             ...
             const int& radius,
             const int& thickness)
      : pt(pt), color(color), radius(radius), thickness(thickness) {}
  cv::Point2f pt;
  cv::Scalar color;
  int radius;
  int thickness;
...
struct FlaskLine {
  FlaskLine(const std::pair<cv::Point2f, cv::Point2f>& pt_pair,
            const cv::Scalar& color,
            const int& thickness)
      : pt_pair(pt_pair), color(color), thickness(thickness) {}
...
struct FlaskText {
  FlaskText(const std::string& str,
            ...
            const cv::Scalar& color,
            const double& scale)
      : str(str), pt(pt), color(color), scale(scale) {}
  std::string str;
  cv::Point2f pt;
  cv::Scalar color;
  double scale;
...
class FlaskStream {
 public:
  ...
  FlaskStream& operator<<(const char* str) {
    this->logs.push_back(std::string(str));
    return *this;
  }
  ...
  FlaskStream& operator>>(cv::Mat& img) {
    ...
    for (auto& str : this->logs) {
      cv::putText(img, str, {20, 80 + cnt * 24}, cv::FONT_HERSHEY_SIMPLEX, 0.8,
                  {0, 0, 255});
      ++cnt;
    }
    ...
    for (auto& line : this->lines)
      cv::line(img, line.pt_pair.first, line.pt_pair.second, line.color,
               line.thickness);
    for (auto& text : this->texts)
      cv::putText(img, text.str, {int(text.pt.x), int(text.pt.y)},
                  cv::FONT_HERSHEY_TRIPLEX, text.scale, text.color);
    return *this;
  }
...
class StatusDebugger {
 public:
  void fix(::RobotStatus& status) const;
...
inline auto start_end_time_to_fmt_pair(const debug::StartEndTime& x)
  ...
  return {fmt::format("{}_到_{}_耗时", x.start, x.end),
          fmt::format("{} 微秒", x.time)};
}

inline auto auto_aim_page() -> std::shared_ptr<base::webview_data::Page> {
  return umt::ObjManager<::base::webview_data::Page>::find(
      "aimer.auto_aim.aim");
}
