...
class SingleFilter {
 public:
  ...
  void init_x(const double& x) {
    this->ekf.init_x(aimer::math::vec_x_to_mat_x1<1>({x}));
  }
  ...
  void update(const double& x,
    ...
    aimer::SinglePredict<ORDER> predict_func{t - this->ekf_t};
    this->ekf.update(measure_func, predict_func,
                     aimer::math::vec_x_to_mat_x1<1>({x}),
                     aimer::math::vec_x_to_mat_xx<SingleFilter::ORDER>(q_vec),
...
class PositionFilter : public PositionPredictorInterface {
 public:
  ...
  Eigen::Vector3d predict_pos(
      const double& t) const override {  // const 这个是不得不加
    Eigen::Vector3d res;
...
class PositionEkf : public PositionPredictorInterface {
 public:
  ...
  Eigen::Vector3d predict_pos(const double& t) const override {
    ...
    return Eigen::Vector3d(pre_res.x_p(0, 0), pre_res.x_p(2, 0),
                           pre_res.x_p(4, 0));
  }
  ...
  Eigen::Vector3d predict_v(const double& t) const override {
    aimer::EkfPredict predict_func{t - this->ekf_t};
    Ekf::PredictResult pre_res = this->ekf.predict(predict_func);
    return Eigen::Vector3d(pre_res.x_p(1, 0), pre_res.x_p(3, 0),
                           pre_res.x_p(5, 0));
  }

  void set_pos(const aimer::math::YpdCoord& ypd) {
  ...
  void update(const aimer::math::YpdCoord& ypd,
              const double& t,
              const std::vector<double>& q_vec,
              const std::vector<double>& r_vec) {
    aimer::EkfMeasure measure_func;
    aimer::EkfPredict predict_func{t - this->ekf_t};
    this->ekf.predict_forward(predict_func,
                              aimer::math::vec_x_to_mat_xx<6>(q_vec));
    Ekf::MeasureResult mea_res = this->ekf.measure(measure_func);
    aimer::math::YpdCoord ypd_p{mea_res.y_e(0, 0), mea_res.y_e(1, 0),
                                mea_res.y_e(2, 0)};
    // predict_forward 之后的内部 ypd 需和数据 ypd 做一次 get_closest，以防
    // (-179) - (+179) 的差角为 358 度。ceres:Jet 是一阶展开，无法处理 358 度
    std::vector<double> ypd_close{
        aimer::math::get_closest(ypd.yaw, ypd_p.yaw, 2. * M_PI),
        aimer::math::get_closest(ypd.pitch, ypd_p.pitch, 2. * M_PI), ypd.dis};
    this->ekf.update_forward(measure_func,
...
class AngleFilter {
 public:
  // 禁用 predict_order
  AngleFilter(const double& range,
              const std::vector<double>& Q,
              const double& R)
      : filter(Kalman<V_Z, V_X>(Q, R, V_X)), range(range) {}

  void update(const double& angle, const double& t) {
    this->filter.update(aimer::math::get_closest(
                            angle, this->filter.predict(t)(0, 0), this->range),
                        t);
    // 强制修正内部更新的角度
    this->filter.set_x(
        aimer::math::reduced(this->filter.get_x_k1()(0, 0), this->range));
  }
  ...
  void init() { this->filter.init(); }

  double get_angle() const { return this->filter.get_x_k1()(0, 0); }

  double get_w() const { return this->filter.get_x_k1()(1, 0); }
