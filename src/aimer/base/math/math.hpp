...
struct YpdCoord {
  ...
  math::YpdCoord operator+(const math::YpdCoord& ypd) {
    return math::YpdCoord(this->yaw + ypd.yaw, this->pitch + ypd.pitch,
                          this->dis + ypd.dis);
  }
...
template <typename T>
T in_range(const T& x, const T& lower, const T& upper) {
  if (x < lower) {
    return lower;
  }
  if (x > upper) {
...
template <typename T>
T in_range(const T& x, const T& lower, const T& upper, const T& default_value) {
  if (x < lower || x > upper) {
...
// 三点求叉积，p1 为原点
inline float get_cross(const cv::Point2f& pt1,
...
// 总是返回 0 ~ pi
inline double get_abs_angle(const Eigen::Vector2d& vec1,
...
// from vec1 to vec2
inline double get_rotate_angle(const Eigen::Vector2d& vec1,
                               const Eigen::Vector2d& vec2) {
...
inline double get_weighted_angle(const double& theta1,
                                 const double& w1,
                                 const double& theta2,
                                 const double& w2) {
...
inline double get_closest(const double& cur,
                          const double& last,
                          const double& range) {
  double reduced = math::reduced(cur, range);
  double possibles[3] = {reduced - range, reduced, reduced + range};
  double closest = possibles[0];
  for (int i = 0; i < 3; ++i)
    if (std::fabs(last - possibles[i]) < std::fabs(last - closest))
      closest = possibles[i];
  return closest;
}
...
inline double get_termination_dis(const double& dis,
                                  const double& angle1,
                                  const double& angle2) {
  return std::fabs(2. * dis *
                   std::sin(math::get_rotate_angle(angle1, angle2) / 2.));
}
...
inline math::Line pt_and_norm_to_line(const Eigen::Vector2d& pt,
                                      const Eigen::Vector2d& norm) {
  return math::Line(norm(1, 0), -norm(0, 0),
                    norm(0, 0) * pt(1, 0) - norm(1, 0) * pt(0, 0));
}

inline Eigen::Vector2d get_intersection(const math::Line& line1,
                                        const math::Line& line2) {
  return Eigen::Vector2d((line1.b * line2.c - line2.b * line1.c) /
                             (line1.a * line2.b - line2.a * line1.b),
                         (line2.a * line1.c - line1.a * line2.c) /
                             (line1.a * line2.b - line2.a * line1.b));
}

inline Eigen::Vector2d get_intersection(const Eigen::Vector2d& pt_a,
...
inline math::YpdCoord xyz_to_ypd(const Eigen::Vector3d& xyz) {
  ...
  ypd.pitch = std::atan2(
      xyz(2, 0), std::sqrt(xyz(0, 0) * xyz(0, 0) + xyz(1, 0) * xyz(1, 0)));
  ypd.dis = xyz.norm();
  return ypd;
}

template <typename T>
void ceres_xyz_to_ypd(T xyz[3], T ypd[3]) {
  ypd[0] = ceres::atan2(xyz[1], xyz[0]);  // yaw
  ypd[1] = ceres::atan2(
      xyz[2], ceres::sqrt(xyz[0] * xyz[0] + xyz[1] * xyz[1]));  // pitch
  ypd[2] = ceres::sqrt(xyz[0] * xyz[0] + xyz[1] * xyz[1] +
...
inline Eigen::Vector3d camera_ypd_to_xyz(
    const math::YpdCoord& ypd) {  // 已验证正确性
  Eigen::Vector3d xyz;
...
template <int N_X>
Eigen::Matrix<double, N_X, N_X> vec_x_to_mat_xx(
    const std::vector<double>& x_vec) {
  Eigen::Matrix<double, N_X, N_X> xx_mat =
      Eigen::Matrix<double, N_X, N_X>::Zero();
  for (int i = 0; i < N_X; ++i) {
    xx_mat(i, i) = x_vec[i];
  }
  return xx_mat;
...
template <int N_X>
Eigen::Matrix<double, N_X, 1> vec_x_to_mat_x1(
    const std::vector<double>& x_vec) {
...
class Bisection {
 public:
  ...
  std::pair<ValueT, ValueT> find(ValueT left,
                                 ValueT right,
                                 Func&& cost_function,
