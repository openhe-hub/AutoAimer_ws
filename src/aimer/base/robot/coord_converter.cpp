...
const double MIN_BULLET_SPEED = 3.;  // m/s
...
class ResistanceFuncLinear {
 private:
  ...
  double get_param_k() const {
    return aimer::param::find_double_param("BULLET_RESISTANCE_K");
  }

  double get_param_m() const {
    return aimer::param::find_double_param("BULLET_RESISTANCE_M");
  }

 public:
  ResistanceFuncLinear(const double& w, const double& h, const double& v0)
      : w(w), h(h), v0(v0) {}
  ...
  bool operator()(const T* const x, T* residual) const {
    ...
    residual[0] =
        (k * this->v0 * ceres::sin(x[0]) + this->g) * k * this->w /
            (k * k * this->v0 * ceres::cos(x[0])) +
        this->g *
            ceres::log(1. - (k * this->w) / (this->v0 * ceres::cos(x[0]))) / k /
            k -
        this->h;
    return true;
  }
...
double CoordConverter::get_yaw_compensate() const {
  return aimer::param::find_int_param("STM32_USING_ANGLE_COMPENSATE") == 1
...
double CoordConverter::get_bullet_speed() const {
  return std::max(double(this->get_robot_status_ref().bullet_speed),
                  aimer::MIN_BULLET_SPEED);
}

double CoordConverter::get_img_to_predict_latency() const {
  double latency =
      this->predict_timestamp_binder.get(this->get_frame()) - this->img_t;
  return aimer::math::in_range(latency, aimer::MIN_IMG_TO_PREDICT_LATENCY,
...
double CoordConverter::get_send_to_control_latency() const {
  return aimer::param::find_double_param("SEND_TO_CONTROL_LATENCY");
}
...
double CoordConverter::get_control_to_fire_latency() const {
  ...
  //     aimer::param::find_double_param("DEFAULT_CONTROL_TO_FIRE_LATENCY"));
  return aimer::param::find_double_param("DEFAULT_CONTROL_TO_FIRE_LATENCY");
}

double CoordConverter::get_fire_to_hit_latency(
...
double CoordConverter::get_img_to_prediction_latency(
    const Eigen::Vector3d& aim_xyz_i_barrel) const {
  return this->get_img_to_predict_latency() +
         this->get_predict_to_send_latency() +
         this->get_send_to_control_latency() +
         this->get_fire_to_hit_latency(aim_xyz_i_barrel);
}

// yaw_v 和 原始 yaw 均采用该时间点的信息
double CoordConverter::get_prediction_time(
    const Eigen::Vector3d& aim_xyz_i_barrel) const {
  return this->get_img_t() +
         this->get_img_to_prediction_latency(aim_xyz_i_barrel);
}

// 此刻给出发弹指令，目标为 aim_pos，则该指令对应子弹的命中延迟
double CoordConverter::get_img_to_hit_latency(
    const Eigen::Vector3d& aim_xyz_i_barrel) const {
  return this->get_img_to_predict_latency() +
         this->get_predict_to_send_latency() +
         this->get_send_to_control_latency() +
         this->get_control_to_fire_latency() +
         this->get_fire_to_hit_latency(aim_xyz_i_barrel);
}

// 此刻给出发弹指令，目标为 aim_pos，则该指令对应子弹的命中时间
double CoordConverter::get_hit_time(
    const Eigen::Vector3d& aim_xyz_i_barrel) const {
...
Eigen::Vector2d CoordConverter::get_camera_z_i_pitch_vec() const {
  ...
  Eigen::Vector2d camera_z_i_pitch_vec{
      std::sqrt(aimer::math::sq(camera_z_i(0, 0)) +
...
double CoordConverter::aim_to_horiz_diff(
    const aimer::math::YpdCoord& ypd) const {
  double yaw_diff = std::fabs(ypd.yaw - this->get_yaw_compensate());
  return aimer::math::get_termination_dis(ypd.dis, yaw_diff);
}

double CoordConverter::aim_to_vert_diff(
    const aimer::math::YpdCoord& ypd) const {
...
bool CoordConverter::aim_error_exceeded(
  ...
  if (this->aim_to_horiz_diff(ypd) >
    ...
    return true;
  if (this->aim_to_vert_diff(ypd) >
      sample_armor.height / 2. *
          std::cos(pitch_inclined_imu + this->get_camera_z_i_pitch()) *
          error_rate)
...
bool CoordConverter::aim_error_exceeded(
  ...
  if (aimer::math::get_termination_dis(ideal_ypd.dis, real_ypd.yaw,
                                       ideal_ypd.yaw) >
      sample_armor.width / 2. * std::cos(yaw_inclined_camera) * error_rate)
    return true;
  if (aimer::math::get_termination_dis(ideal_ypd.dis, real_ypd.pitch,
                                       ideal_ypd.pitch) >
      sample_armor.height / 2. *
          std::cos(pitch_inclined_world + this->get_camera_z_i_pitch()) *
          error_rate)
    // 对方朝上 15 度，我朝下 15 度，这是刚好对准的
    return true;
  return false;
...
bool CoordConverter::aim_cmp(const aimer::AimInfo& aim_a,
                             const aimer::AimInfo& aim_b) const {
  if ((aim_a.shoot == ::ShootMode::IDLE) != (aim_b.shoot == ::ShootMode::IDLE))
    return (aim_a.shoot == ::ShootMode::IDLE) <
           (aim_b.shoot == ::ShootMode::IDLE);  // false 优先
...
cv::Point2f CoordConverter::pd_to_pu(const cv::Point2f& pd) const {
  ...
  cv::undistortPoints(pds, pus, this->f_cv_mat, this->c_cv_mat, cv::noArray(),
                      this->f_cv_mat);
  return pus[0];
}
...
aimer::math::YpdCoord CoordConverter::get_camera_ypd_v(
  ...
  return aimer::math::camera_get_ypd_v(this->pi_to_pc(xyz_i),
                                       this->pi_to_pc(xyz_v_i));
}

// pts 转标准化 pis
std::vector<Eigen::Vector3d> CoordConverter::pts_to_pis_norm(
    const std::vector<cv::Point2f>& pts) const {
  std::vector<cv::Point2f> pus;
  cv::undistortPoints(pts, pus, this->f_cv_mat, this->c_cv_mat, cv::noArray(),
                      this->f_cv_mat);
  std::vector<Eigen::Vector3d> pis_norm;
  for (std::size_t i = 0; i < pus.size(); ++i) {
    pis_norm.push_back(this->pc_to_pi(this->pu_to_pc_norm(pus[i])));
  }
  return pis_norm;
}

cv::Point2f CoordConverter::aim_ypd_to_pu(
    const aimer::math::YpdCoord& ypd) const {
  Eigen::Vector3d pc = aimer::math::camera_ypd_to_xyz(ypd);
  return this->pc_to_pu(pc);
}

Eigen::Vector3d CoordConverter::offset_parabola(
...
//       ypd_camera.yaw -
//           aimer::param::find_double_param("CAMERA_TO_BARREL_YAW") / 180. *
//           M_PI,
//       ypd_camera.pitch -
//           aimer::param::find_double_param("CAMERA_TO_BARREL_PITCH") / 180. *
//           M_PI,
//       ypd_camera.dis);
...
//   aimer::param::find_double_param("CAMERA_TO_BARREL_Y");
...
//   pc_fix_y(1, 0) += aimer::param::find_double_param("CAMERA_TO_BARREL_Y");
...
Eigen::Vector3d CoordConverter::xyz_i_camera_to_xyz_i_barrel(
...
Eigen::Vector3d CoordConverter::xyz_i_barrel_to_xyz_i_camera(
    const Eigen::Vector3d& xyz_i_barrel) const {
  const Eigen::Vector3d xyz_c_barrel = this->pi_to_pc(xyz_i_barrel);
  const Eigen::Vector3d xyz_c_camera = {
      Eigen::Vector3d(xyz_c_barrel(0, 0),
                      xyz_c_barrel(1, 0) +
                          aimer::param::find_double_param("CAMERA_TO_BARREL_Y"),
                      xyz_c_barrel(2, 0))};
...
aimer::ShootParam CoordConverter::target_pos_to_shoot_param(
    const Eigen::Vector3d& target_pos) const {
  // Eigen::Vector3d pos = raw_pos;
  Eigen::Vector3d target_xyz_i_barrel =
      this->xyz_i_camera_to_xyz_i_barrel(target_pos);
  const double bs = this->get_bullet_speed();
  const double target_xy =
      std::sqrt(target_xyz_i_barrel(0, 0) * target_xyz_i_barrel(0, 0) +
...
aimer::math::YpdCoord CoordConverter::aim_xyz_i_to_aim_ypd(
    const Eigen::Vector3d& aim_xyz_i) const {
  Eigen::Vector3d aim_xyz_c_barrel = this->pi_to_pc(aim_xyz_i);
  aimer::math::YpdCoord aim_ypd_c_barrel =
      aimer::math::camera_xyz_to_ypd(aim_xyz_c_barrel);
  aimer::math::YpdCoord aim_ypd_b{
      aim_ypd_c_barrel.yaw -
          aimer::param::find_double_param("AIM_YAW_OFFSET") / 180. * M_PI,
      aim_ypd_c_barrel.pitch -
          aimer::param::find_double_param("AIM_PITCH_OFFSET") / 180. * M_PI,
...
aimer::math::YpdCoord CoordConverter::target_pos_to_aim_ypd(
    const Eigen::Vector3d& pos) const {
  Eigen::Vector3d aim_xyz_i_barrel =
      this->target_pos_to_shoot_param(pos).aim_xyz_i_barrel;
  return this->aim_xyz_i_to_aim_ypd(aim_xyz_i_barrel);
  // return this->offset_camera_to_barrel(aimer::math::camera_xyz_to_ypd(pc));
}

double CoordConverter::filter_to_prediction_time(
    const PositionPredictorInterface& filter) const {
  int iterations_num =
      aimer::param::find_int_param("PREDICTION_ITERATIONS_NUM");
  double prediction_t = this->get_img_t();
  for (int i = 0; i < iterations_num; ++i)
    prediction_t = this->get_prediction_time(
        this->target_pos_to_shoot_param(filter.predict_pos(prediction_t))
            .aim_xyz_i_barrel);
  return prediction_t;
}

aimer::ShootParam CoordConverter::filter_to_shoot_param(
    const PositionPredictorInterface& filter) const {
...
// aim 总是关于枪口的，这里存在误差
aimer::math::YpdCoord CoordConverter::filter_to_aim_ypd_v(
    const PositionPredictorInterface& filter) const {
  double prediction_t = this->filter_to_prediction_time(filter);
  return this->get_camera_ypd_v(filter.predict_pos(prediction_t),
                                filter.predict_v(prediction_t));
}

double CoordConverter::filter_to_hit_time(
    const PositionPredictorInterface& filter) const {
  int iterations_num =
      aimer::param::find_int_param("PREDICTION_ITERATIONS_NUM");
  ...
  for (int i = 0; i < iterations_num; ++i) {
    hit_t = this->get_hit_time(
        this->target_pos_to_shoot_param(filter.predict_pos(hit_t))
            .aim_xyz_i_barrel);
  }
  return hit_t;
}

Eigen::Vector3d CoordConverter::filter_to_hit_pos(
    const PositionPredictorInterface& filter) const {
  return filter.predict_pos(this->filter_to_hit_time(filter));
}

aimer::math::YpdCoord CoordConverter::filter_to_hit_aim_ypd(
    const PositionPredictorInterface& filter) const {
  return this->target_pos_to_aim_ypd(this->filter_to_hit_pos(filter));
}

// aim 总是关于枪口的，这里存在误差
aimer::math::YpdCoord CoordConverter::filter_to_hit_aim_ypd_v(
    const PositionPredictorInterface& filter) const {
...
void CoordConverter::update(const cv::Mat& img,
  ...
  if (aimer::param::find_int_param("STATUS_DEBUGGER_ON") == 1) {
    this->status_debugger.fix(*this->robot_status_obj);
  }
...
CreditClock::CreditClock(aimer::CoordConverter* const converter,
                         const double& credit_time)
    : converter(converter), credit_time(credit_time) {}
